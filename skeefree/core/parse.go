...
func dissectAlterTableStatement(statement string) (tableName string, alter string, err error) {
	submatch := alterTableRegexp.FindStringSubmatch(statement)
	if len(submatch) == 0 {
		return "", "", fmt.Errorf("cannot dissect ALTER statement: %s", statement)
	}
	tableName = submatch[1]
	alter = submatch[2]
	return tableName, alter, nil
}